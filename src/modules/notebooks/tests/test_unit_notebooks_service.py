...
def mock_notebook_repo():
    """
    Esta fixture cria um mock para o notebook_repository.
    O 'yield' entrega o mock para o teste e garante que ele seja "limpo" depois.
    """
...
class TestUnitNotebookService:
    ...
    def test_create_notebook_success(self, mock_notebook_repo: MagicMock):
        """
        Testa se o serviço chama o repositório corretamente para criar um notebook
        ...
        mock_notebook_repo.create_notebook.return_value = Notebook(
            id=uuid.uuid4(), name=notebook_data.name
        )

        result = NotebookService.create_notebook(mock_db_session, notebook_data)

        mock_notebook_repo.create_notebook.assert_called_once_with(
            mock_db_session, notebook_data
        )
        assert result.name == notebook_data.name

    def test_create_notebook_duplicate_raises_409(self, mock_notebook_repo: MagicMock):
        """
        Testa se o serviço levanta um HTTPException 409 quando o repositório
        sinaliza um erro de integridade (nome duplicado).
        """
        ...
        with pytest.raises(HTTPException) as exc_info:
            NotebookService.create_notebook(mock_db_session, notebook_data)
    ...
    def test_get_notebook_by_id_success(self, mock_notebook_repo: MagicMock):
        """
        Testa se o serviço retorna um notebook encontrado pelo repositório.
        """
        notebook_id = uuid.uuid4()
        mock_db_session = MagicMock()
        mock_notebook_repo.get_notebook_by_id.return_value = Notebook(
            id=notebook_id, name="Caderno Encontrado"
        )
        result = NotebookService.get_notebook_by_id(mock_db_session, notebook_id)
    ...
    def test_get_notebook_by_id_not_found_raises_404(
        ...
        """
        Testa se o serviço levanta um HTTPException 404 quando o repositório
        retorna None.
        """
        notebook_id = uuid.uuid4()
        mock_db_session = MagicMock()
        ...
        with pytest.raises(HTTPException) as exc_info:
            NotebookService.get_notebook_by_id(mock_db_session, notebook_id)
    ...
    def test_udpate_notebook_data_by_id(
        ...
        """Testa a atualização de um notebook com diferentes dados."""
        notebook_id = uuid.uuid4()
        mock_db_session = MagicMock()
        notebook_to_update = Notebook(
            id=notebook_id, name=initial_name, is_favorite=False
        )
        ...
        result = NotebookService.update_notebook_data_by_id(
            mock_db_session, notebook_id, update_data
        )
        assert result.name == expected_name
        assert result.is_favorite == expected_favorite

    def test_delete_notebook_by_id(self, mock_notebook_repo: MagicMock):
        """Testa se o serviço chama corretamente o repositório para deletar."""
        notebook_id = uuid.uuid4()
        mock_db_session = MagicMock()
        notebook_to_delete = Notebook(id=notebook_id, name="Para Deletar")

        mock_notebook_repo.get_notebook_by_id.return_value = notebook_to_delete
        NotebookService.delete_notebook_by_id(mock_db_session, notebook_id)
